...
const useVideo = (videoId: string) => {
  ...
  const fetchData = async (refresh: boolean = false) => {
    ...
    try {
      ...
      console.log('📞 Making API call to:', `http://localhost:8000/dashboard/videos/${videoId}?refresh=${refresh}`);
      const response = await fetch(`http://localhost:8000/dashboard/videos/${videoId}?refresh=${refresh}`, {
        headers: {
